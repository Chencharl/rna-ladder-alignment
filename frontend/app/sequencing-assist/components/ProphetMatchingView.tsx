        ...
        <div className="flex flex-wrap gap-0.5">
          {rows.map((r) => (
            <div
              ...
              className={`rounded-sm ${!r.in_range_5p ? "bg-gray-100" : r.hit_5p ? "bg-blue-500" : "bg-gray-200"}`}
            />
          ))}
        </div>
      </div>
        ...
        <div className="flex flex-wrap gap-0.5">
          {rows.map((r) => (
            <div
              ...
              style={{ width: CELL_W, height: CELL_W }}
              className={`rounded-sm ${!r.in_range_3p ? "bg-gray-100" : r.hit_3p ? "bg-purple-500" : "bg-gray-200"}`}
            />
          ))}
        </div>
      </div>
      <div className="flex flex-wrap gap-4 mt-2 text-xs text-gray-400">
        <span className="flex items-center gap-1"><span className="inline-block w-3 h-3 rounded-sm bg-blue-500" /> 5′ hit</span>
        <span className="flex items-center gap-1"><span className="inline-block w-3 h-3 rounded-sm bg-purple-500" /> 3′ hit</span>
        <span className="flex items-center gap-1"><span className="inline-block w-3 h-3 rounded-sm bg-gray-200" /> not detected</span>
        <span className="flex items-center gap-1"><span className="inline-block w-3 h-3 rounded-sm bg-gray-100" /> outside 2–23 kDa</span>
      </div>
    </div>
  );
}

function ProphetContent({ result }: { result: ProphetMatchingResult }) {
  ...
  const filtered = useMemo(() => {
    return result.rows.filter((r) => {
      if (missOnly) {
        return (r.in_range_5p && !r.hit_5p) || (r.in_range_3p && !r.hit_3p);
      }
      return true;
    });
  }, [result, missOnly]);
    ...
    <div>
      ...
      <div className="flex flex-wrap gap-3 mb-5">
        ...
        <div className="rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-center min-w-[90px]">
          ...
          <div className="text-xs text-blue-500">longest consecutive 5′</div>
        </div>
        <div className="rounded-lg border border-purple-200 bg-purple-50 px-3 py-2 text-center min-w-[90px]">
          ...
          <div className="text-xs text-purple-500">longest consecutive 3′</div>
        </div>
      ...
      <div className="flex flex-wrap gap-3 mb-3 items-center">
        ...
        {(["both", "5p", "3p"] as const).map((v) => (
          <button
            ...
            className={`text-xs px-2 py-1 rounded border transition-colors ${showLadder === v
        ...
        <button
          ...
          className={`text-xs px-2 py-1 rounded border transition-colors ${missOnly
              ...
              <tr key={r.position} className="border-b border-gray-50 hover:bg-gray-50/50">
                ...
                {(showLadder === "both" || showLadder === "5p") && <>
                  ...
                  <td className="py-1.5 px-2 tabular-nums font-mono">
                    {r.hit_5p ? <span className="text-blue-700 font-medium">{r.obs_mass_5p?.toFixed(4)}</span>
                      : r.in_range_5p ? <span className="text-gray-300">NA</span>
                      : <span className="text-gray-200">—</span>}
                  </td>
                  <td className="py-1.5 px-2 tabular-nums text-right text-gray-500">{r.hit_5p ? r.delta_mda_5p?.toFixed(1) : "—"}</td>
                  <td className="py-1.5 px-2 tabular-nums text-right text-gray-500">
                    {r.hit_5p && r.obs_rel_i_5p != null ? (r.obs_rel_i_5p * 100).toFixed(1) + "%" : "—"}
                  </td>
                  <td className="py-1.5 px-2 tabular-nums text-right text-gray-500">{r.hit_5p ? r.obs_rt_5p?.toFixed(2) : "—"}</td>
                  <td className="py-1.5 px-2 text-center">
                    {!r.in_range_5p ? <span className="text-gray-300 text-xs">○</span>
                      : r.hit_5p ? <span className="text-blue-600 font-bold">✓</span>
                ...
                {(showLadder === "both" || showLadder === "3p") && <>
                  ...
                  <td className="py-1.5 px-2 tabular-nums font-mono">
                    {r.hit_3p ? <span className="text-purple-700 font-medium">{r.obs_mass_3p?.toFixed(4)}</span>
                      : r.in_range_3p ? <span className="text-gray-300">NA</span>
                      : <span className="text-gray-200">—</span>}
                  </td>
                  <td className="py-1.5 px-2 tabular-nums text-right text-gray-500">{r.hit_3p ? r.delta_mda_3p?.toFixed(1) : "—"}</td>
                  <td className="py-1.5 px-2 tabular-nums text-right text-gray-500">
                    {r.hit_3p && r.obs_rel_i_3p != null ? (r.obs_rel_i_3p * 100).toFixed(1) + "%" : "—"}
                  </td>
                  <td className="py-1.5 px-2 tabular-nums text-right text-gray-500">{r.hit_3p ? r.obs_rt_3p?.toFixed(2) : "—"}</td>
                  <td className="py-1.5 px-2 text-center">
                    {!r.in_range_3p ? <span className="text-gray-300 text-xs">○</span>
                      : r.hit_3p ? <span className="text-purple-600 font-bold">✓</span>
                      : <span className="text-gray-300">✗</span>}
                  </td>
                </>}
              </tr>
      ...
      {filtered.length === 0 && <EmptyState>No rows match the current filter.</EmptyState>}
...
interface ProphetMatchingViewProps {
  presetResults?: Record<string, ProphetMatchingResult> | null;
  customResult?: ProphetMatchingResult | null;
}

export function ProphetMatchingView({ presetResults, customResult }: ProphetMatchingViewProps) {
  // Build ordered tab list: presets first, then custom reference if different
  const tabs: Array<{ label: string; result: ProphetMatchingResult; badge?: string }> = [];

  if (presetResults) {
    for (const [label, res] of Object.entries(presetResults)) {
      if (res?.rows?.length) {
        tabs.push({ label, result: res });
      }
    }
  }
  if (customResult?.rows?.length) {
    tabs.push({ label: "Custom Reference", result: customResult, badge: "custom" });
  }
  ...
  return (
    <Card
      title="Reference-Guided Ladder Matching"
      subtitle={`Position-by-position comparison of theoretical and observed fragment masses. ${tabs.length > 1 ? `${tabs.length} reference sequences matched simultaneously.` : ""} Tolerance ±${active.result.tolerance_da * 1000} mDa.`}
    >
      {/* Tab bar — only shown when multiple species */}
      {tabs.length > 1 && (
        <div className="flex gap-1 mb-5 border-b border-gray-100 overflow-x-auto">
          {tabs.map((t, i) => (
            ...
            >
              {t.label}
              {t.badge && (
                <span className="ml-1.5 text-[10px] bg-gray-100 text-gray-500 rounded px-1 py-0.5">{t.badge}</span>
              )}
            </button>
          ))}
        </div>
      ...
      <ProphetContent result={active.result} />
    </Card>
  );
}
