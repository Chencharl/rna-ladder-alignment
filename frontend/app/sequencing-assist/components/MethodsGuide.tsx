...
const SECTIONS = [
  ...
  },
  {
    ...
    body: `Seeds are the highest-intensity (block-normalized Rel_I) unassigned peaks. Each seed is extended greedily in both mass directions by matching consecutive peak-to-peak mass differences against the modification dictionary. Each extension step may span up to 3 residue masses (max_residues_per_step = 3), allowing the algorithm to bridge up to 2 missing ladder rungs in one jump — peaks that are absent due to poor ionization or co-elution. Gap steps that skip residues are flagged visually and show the combined mass delta. The default chain-building tolerance is ±0.05 Da. A monotonic RT trend filter (minimum 3-point history; RT standard deviation floor 0.10 min, giving ±0.25 min per step) rejects physically implausible extensions and prevents cross-species RT drift artifacts in multi-species mixtures. Block-wise Rel_I normalization — computed within 320 Da mass windows — prevents high-density mass regions from monopolising the seed queue.`,
  },
  {
  ...
  },
  {
    ...
    body: `The following fixed parameters govern chain recovery and can be cited directly in a paper methods section:\n\n• Block window: 320 Da (Rel_I normalisation)\n• Chain-building mass tolerance: ±0.05 Da\n• Post-hoc decoding tolerance: ±0.07 Da\n• Max residues per extension step: 3 (Dr. Jiang strict; allows ≤2 missing positions per jump)\n• RT trend filter: monotonic; std-floor 0.10 min (±0.25 min/step); min 3-point history\n• Modification dictionary: 47 entries (4 canonical + 43 modifications)\n• Default seed threshold: top 5% of block-normalised Rel_I\n• Default minimum read length: 10 positions\n• FDR null model: random peak-pair Δ mass sampling, same ±0.05 Da window\n\nSuggested citation text:\n"De-novo sequence reads were recovered from deconvoluted monoisotopic masses using a nested ladder alignment algorithm. Peaks were normalised within 320 Da mass windows; seeds were extended greedily by matching consecutive mass differences against a 47-entry RNA modification dictionary (±0.05 Da tolerance; up to 3 residues per step, allowing ≤2 missing ladder rungs). A monotonic RT trend filter (std-floor 0.10 min; min 3-point history) was applied. 5′/3′ terminus assignment was based on terminal-nucleotide decimal mass signatures, retention-time correlation, and precursor mass closure scoring. Reads ≥10 positions were retained; empirical FDR was estimated by random peak-pair sampling. Input peaks were cross-referenced against theoretical 5′ and 3′ ladder masses computed from a 46-family human tRNA reference library."`,
  },
];
